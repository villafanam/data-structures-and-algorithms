...
const maxInArray = (arr) => {
  // Solution code here...
  return arr.reduce((max,value) => max = Math.max(max, value),0);
};
...
const hasChildrenValues = (arr, character) => {
  // Solution code here...
  let charObj = arr.find(element => element.name === character);
  let valArr = Object.values(charObj);

  return valArr.length === 4 ? true : false;
};
