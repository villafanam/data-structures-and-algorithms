...
let biggerThanLuke = (arr) => {
  // Solution code here...
  return arr.filter(element => element.mass > 77).map(person => person.name).join(' - ');
};
...
const detectTicTacToeWin = (board) => {
  ...
  {
    if (board[0][j] === board[1][j] && board[1][j] === board[2][j] && board[0][j] === board[2][j] && board[2][j] !== '')
    {
      return true;
    }
  }
  ...
  //diagonals
  if (board[0][0] === board[1][1] && board[1][1] === board[2][2] && board[0][0] === board[2][2] && board[2][2] !== '')
  {
    return true;
  }
  else
