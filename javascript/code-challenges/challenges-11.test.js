...
function transformToLis(obj) {
  ...
  for (let i = 0; i < objArr.length; i++) {
    newArr.push(`<li>${objArr[i][0]}: ${objArr[i][1]}</li>`);
  }
...
const count = (target, input) => {
  // Solution code here...
  return input.flat(2).reduce((count, value) => value === target ? count += 1 : count += 0, 0);
};
...
const totalSum = (input) => {
  // Solution code here...
  return input.flat().reduce((sum, value) => sum += value, 0);
};
...
const divisibleByFiveTwoToThePower = (input) => {
  ...
  let newArr = input.map(element => {
    return (element.filter(elem => Number.isInteger(elem) && elem));
  });

  newArr = newArr.map(element => {
    return (element.filter(elem => elem % 5 === 0 && elem));
  });

  newArr = newArr.map(element => {
    if (element.length === 0) { return []; }
    return element.map(elem => Math.pow(2, elem));
  });
...
let findMaleAndFemale = (data) => {
  // Solution code here...
  return data.reduce((str, element, idx) => {
...
let findShortest = (data) => {
  // Solution code here...
  let height = data.reduce((height, element) => element.height > height ? element.height : height,0);
  let star = data.filter(element => element.height === height);
  return star[0].name;
};
